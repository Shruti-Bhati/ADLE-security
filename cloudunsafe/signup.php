...
mysql_select_db("user_detailsunsafe",$con) or die(mysql_error());

  if(isset($_POST['username_cloud'])&&($_POST['password_cloud'])&&($_POST['email']))
  {
  $username = $_POST['username_cloud'];
  $password = $_POST['password_cloud'];
  $email = $_POST['email'];
  $sql = "SELECT * FROM userunsafe WHERE username = \"$username\" ";
  if(mysql_num_rows(mysql_query($sql,$con))==0)
  {
	$query = "INSERT INTO userunsafe (username, password, email)VALUES ('$username', '$password','$email')";
